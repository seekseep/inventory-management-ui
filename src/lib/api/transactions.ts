...
export const transactionKeys = {
  ...
  listWithItems: () => [...transactionKeys.all, 'listWithItems'] as const,
}
...
export function useTransactionsWithItems() {
  const { data: transactions, isLoading: isListLoading } = useTransactions()

  const detailQueries = useQueries({
    queries: (transactions ?? []).map((tx) => ({
      queryKey: transactionKeys.detail(tx.id),
      queryFn: () => getTransaction(tx.id),
      staleTime: DEFAULT_STALE_TIME,
    })),
  })
  ...
  const data = isLoading
    ...
    : detailQueries
        .map((q) => q.data)
        .filter((d): d is TransactionWithItems => d != null)
